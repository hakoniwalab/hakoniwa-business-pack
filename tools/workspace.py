...
def _render_posix_activate(paths: WorkspacePaths) -> str:
    ...
    if platform.system() == "Linux":
        lines.append(
            f"export LD_LIBRARY_PATH={shlex.quote(str(paths.foundation_lib))}:${{LD_LIBRARY_PATH-}}"
        )
    elif platform.system() == "Darwin":
        lines.append(
            f"export DYLD_LIBRARY_PATH={shlex.quote(str(paths.foundation_lib))}:${{DYLD_LIBRARY_PATH-}}"
        )
    lines.extend(
        [
            "",
...
def validate_snapshot(snapshot: Mapping[str, object], paths: WorkspacePaths) -> list[str]:
    errors: list[str] = []
    if not _is_under(snapshot.get("executable"), paths.foundation_python_root):
        errors.append(
    ...
    for name in expected:
        origin = modules.get(name) if isinstance(modules, Mapping) else None
        if not _is_under(origin, paths.foundation_root):
            errors.append(
                f"{name} resolved outside Foundation workspace: {origin or 'missing'}"
            )
    return errors
...
def enter(paths: WorkspacePaths) -> int:
    ...
    if os.name == "nt":
        ...
        command = (
            [shell, "-NoExit"]
            if shell and "cmd" not in Path(shell).name.lower()
            else [shell]
        )
    else:
        shell = env.get("SHELL") or shutil.which("bash") or shutil.which("sh")
        command = [shell, "-i"] if shell else []
